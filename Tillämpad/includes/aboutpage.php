
<img src="img/weather.jpg" id= img alt="väder">
<section id="start">
    <h2>Om oss:</h2>
        <h4>
            Vi på WeatherCompare erbjuder korrekt och tydligt gjorda grafer som visar olika sorters<br> 
            väderdata som jämför städers väderförhållanden i realtid och snar framtid. <br> 
            All data är hämtat från SMHI:s öppna API-tjänst <br/><br/>
            Vår vision för dig är att du som användare ska få en verklig bild av väderförhållanden på olika platser inom Sverige.<br/><br/>
        </h4>
    <h2>Kontaktuppgifter:</h2>
        <h4>
            ered2000@student.miun.se; Erik Edvall Ung<br/>
            rali2001@student.miun.se; Rasmus Linnarsson <br/>
            tori2000@student.miun.se; Tomas Ringefors <br/>
        </h4>
        
</section>
